...
class Practica
{
    ...
    static int NuevoJuego(JuegoMesa[] juegos, ref int cantidad)
    {
        ...
        {
            JuegoMesa nuevo = new JuegoMesa();
            string nombre;
            int minEdad, minJugadores, maxJugadores;
            float precio;
            ...
            minEdad = Convert.ToInt32(Console.ReadLine());
            ...
            try
            {
                ...
                nuevo.SetTipo(tipo);
                nuevo.SetMinEdad(minEdad);
                nuevo.SetMinJugadores(minJugadores);
                nuevo.SetMaxJugadores(maxJugadores);
    ...
    static void JuegosMasCaro(JuegoMesa[] juegos, int cantidad)
    {
        if (cantidad > 0)
        {
            ...
            {
                if (juegos[i].GetPrecio() > juegoMasCaro.GetPrecio())
                {
        ...
        JuegoMesa.tipoJuego mostrarTipo)
    {
        ...
        mostrarTipo <= JuegoMesa.tipoJuego.OTROS)
        {
            ...
            for (int i = 0; i < cantidad; i++)
            {
                if (mostrarTipo == juegos[i].GetTipo())
                {
    ...
    static void OrdenarJuegos(JuegoMesa[] juegos, int cantidad)
    {
        if (cantidad > 0)
        {
            for (int i = 0; i < cantidad - 1; i++)
            {
                ...
                {
                    if (string.Compare(juegos[i].GetNombre(),
                        juegos[j].GetNombre()) > 0)
                    {
